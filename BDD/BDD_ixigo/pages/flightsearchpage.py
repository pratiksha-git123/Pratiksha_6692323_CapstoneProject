"""
FlightSearchPage — fills in the search form on ixigo.com/flights.
Handles: From city, To city, Departure date, Search button.
"""
import time
...
class FlightSearchPage(BasePage):
    ...
    # ── Locators ────────────────────────────────────────────────────
    # Absolute XPaths (fallback — fragile but work when page layout is stable)
    FROM_INPUT_XPATH = (
    ...
    CARET_INPUT_CSS = "input[class*='caret-selection']"
    ...
    def open_search_page(self):
        self.open(self.URL)
        # Wait for search button to be present (page fully loaded)
    ...
    def wait_for_search_form(self, timeout: int = 15):
        """Wait until the search form is interactive."""
    ...
    def select_departure_date(self, travel_date: date):
        month_year = travel_date.strftime("%B %Y")     # "June 2026"
        day = travel_date.day
        ...
        for _ in range(14):
            try:
                ...
                if any(month_year in lbl.text for lbl in labels):
                    break
                next_btn = self.driver.find_element(By.CSS_SELECTOR, self.CALENDAR_NEXT_CSS)
        ...
        try:
            day_btn = self.find_clickable(By.XPATH, tile_xpath, timeout=5)
            self.safe_click(day_btn)
            log.info(f"Selected date: {aria_label}")
            return
        except TimeoutException:
            pass

        # Fallback: click abbr directly
        try:
            abbr = self.driver.find_element(
                By.CSS_SELECTOR, f"abbr[aria-label='{aria_label}']"
            )
    ...
    def click_search(self):
        ...
        self.dismiss_all_popups()
        btn = self.find_clickable(By.XPATH, self.SEARCH_BTN_XPATH)
        self.safe_click(btn)
    ...
    def _pick_city_option(self, city: str, code: str):
        ...
        log.info(f"Pressing Enter to accept suggestion for {code}.")
