...
class ResultPage(BasePage):
    ...
    def get_flight_count_text(self) -> str:
        visible_count_texts = []
        for element in self.find_all(By.XPATH, self.FLIGHT_COUNT_XPATH):
            try:
                text = element.text.strip()
                if element.is_displayed() and self._parse_available_flight_count(text) > 0:
    ...
    def has_visible_book_buttons(self) -> bool:
        """Return True when at least one visible Book button is present."""
        return any(btn.is_displayed() for btn in self.find_all(By.XPATH, self.BOOK_BTN_XPATH))
    ...
    def click_book(self, index: int = 0):
        ...
        if index < len(visible):
            ...
            try:
                ...
                time.sleep(2)  # fallback if URL doesn't change fast
        else:
