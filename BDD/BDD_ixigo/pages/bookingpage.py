...
class BookingPage(BasePage):
    ...
    def wait_for_booking_page(self, timeout: int = 30):
        ...
        WebDriverWait(self.driver, timeout).until(
            lambda d: (
                "/flight/booking/" in d.current_url.lower()
                or "booking" in d.current_url.lower()
                or "review" in d.current_url.lower()
                or "traveller" in d.page_source.lower()
                or "passenger" in d.page_source.lower()
                or "contact details" in d.page_source.lower()
            )
        )
    ...
    def has_form_validation_error(self) -> bool:
        """Return True when traveller-form validation text is visible."""
